...
class RectangularRoom(object):
    ...
    def cleanTileAtPosition(self, pos):
        ...
        position = (x,y)
        if position not in self.mark:
            self.mark.append(position)
    ...
    def isTileCleaned(self, m, n):
        ...
        """
        try:
            type(m) == int and type(n) == int
        except:
            raise TypeError
        return (m,n) in self.mark
...
def runSimulation(num_robots, speed, width, height, min_coverage, num_trials,
    ...
    for i in range(num_trials):
        ...
        < min_coverage:
            ...
            for robot in botList:
                robot.updatePositionAndClean()
        #result = float(room.getNumCleanedTiles()) / float(room.getNumTiles())
            steps += 1
...
class RandomWalkRobot(Robot):
    ...
    def updatePositionAndClean(self):
        ...
        """
        position = self.position
        newPosition = self.position.getNewPosition(random.randint(0,359),\
        self.speed)
        if self.room.isPositionInRoom(newPosition):
            self.setRobotPosition(newPosition)
            self.room.cleanTileAtPosition(self.position)
